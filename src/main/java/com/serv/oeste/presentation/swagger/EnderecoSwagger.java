...
public interface EnderecoSwagger extends SwaggerConfiguration {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "404", description = "[NotFound] O Endereço informado não foi encontrado.", content = @Content(schema = @Schema(implementation = DomainException.class))),
            @ApiResponse(responseCode = "500", description = "[ServiceUnavailable] A API ViaCep não se encontra online.", content = @Content(schema = @Schema(implementation = DomainException.class))),
            @ApiResponse(responseCode = "503", description = "[InternalServerError] Não foi possível acessar o serviço da ViaCep ou o RestTemplate não está funcionando corretamente(Chamar um técnico).", content = @Content(schema = @Schema(implementation = DomainException.class))),
    })
