...
public interface ServicoSwagger extends SwaggerConfiguration {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "400", description = "[BadRequest] Alguma informação foi passada de forma errada.", content = @Content(schema = @Schema(implementation = DomainException.class))),
            @ApiResponse(responseCode = "404", description = "[NotFound] Algum serviço informado não foi encontrado.", content = @Content(schema = @Schema(implementation = DomainException.class))),
    })
