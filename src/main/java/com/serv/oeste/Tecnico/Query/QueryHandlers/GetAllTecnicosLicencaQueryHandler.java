...
@Service
public class GetAllTecnicosLicencaQueryHandler implements Query<Void, List<Tecnico>> {

    @Autowired private TecnicoRepository tecnicoRepository;
    @Override
    public ResponseEntity<List<Tecnico>> execute(Void unused) {
        List<Tecnico> tecnicos = tecnicoRepository.findAllBySituacao(Situacao.LICENCA);
